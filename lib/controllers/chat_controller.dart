...
class ChatController extends GetxController {
  ...
  Future<String> enviarMensagem(
      String mensagem, String idVendedor, String idProduto) async {
    String mensagemErro = '';
    _loading.value = true;

    try {
      DocumentSnapshot vendedorDoc =
          await _firestore.collection('vendedores').doc(idVendedor).get();

      DocumentSnapshot compradorDoc = await _firestore
  ...
  Stream<QuerySnapshot> obterChat(String idVendedor, String idProduto) {
    return _firestore
        .collection('chats')
        .where('id_comprador', isEqualTo: _auth.currentUser!.uid)
        .where('id_vendedor', isEqualTo: idVendedor)
